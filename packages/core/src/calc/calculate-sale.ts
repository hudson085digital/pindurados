...
export function calculateSale(input: CalculateSaleInput): CalculateSaleResult {
  ...
  if (customValues && customValues.length > 0) {
    ...
    // O valor final é o total que o cliente paga (entrada + parcelas).
    // As parcelas distribuem apenas o que resta depois da entrada, então o
    // total parcelado é o valor final menos a entrada.
    const targetTotalInCents = Math.round(input.targetTotalInCents ?? 0)
    totalInCents = Math.max(0, targetTotalInCents - downPaymentInCents)
